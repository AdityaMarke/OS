...
void srtf_preemptive(struct Process *processes, int n);
...
int main() {
    ...
    while (1) {
        ...
        printf("3. Shortest Remaining Time First (srtf_preemptive)\n");
...
void srtf_preemptive(struct Process *processes, int n) {
    ...
    display_results(processes, n, "srtf_preemptive");
}
...
void round_robin(struct Process *processes, int n, int quantum) {
    ...
    while (completed != n) {
        if (front == rear) {
            ...
            for (int i = 0; i < n; i++) {
                if (remaining_time[i] > 0 && !is_in_queue[i] && processes[i].arrival_time < next_arrival) {
        ...
        int execution_time = (remaining_time[current_process] < quantum) ? 
        ...
        for (int i = 0; i < n; i++) {
            if (remaining_time[i] > 0 && !is_in_queue[i] && 
                processes[i].arrival_time <= current_time && i != current_process) {
                queue[rear] = i;
                is_in_queue[i] = 1;
                rear++;
            }
